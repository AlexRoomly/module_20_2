...
class PriceMachine():
        ...
        for fname in os.listdir(file_path):
            if keyword in fname:
                ...
                with open(fname,'r') as csvfile:
                    ...
                    df_f_del=dialect.delimiter
                    # print(f'Имя файла {fname}', ' delimiter= ', df_f_del)
                    if df_f_del==',':
                        print(f'Формат файла {fname} не соотвествует условию: данные, разделенные точкой с запятой.')
                        continue
    ...
    def _search_product_price_weight(self, headers):
        ...
        for header in headers:
            if header in cols_name: ind_name1 = headers.index(header)
            if header in cols_price: ind_price1 = headers.index(header)
            if header in cols_weight: ind_weight1 = headers.index(header)
        return (ind_name1, ind_price1, ind_weight1)
